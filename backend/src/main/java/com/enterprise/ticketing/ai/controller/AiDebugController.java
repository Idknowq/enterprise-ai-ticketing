...
public class AiDebugController {
    ...
    public AiDebugController(AiOrchestrationService aiOrchestrationService) {
        this.aiOrchestrationService = aiOrchestrationService;
    }

    @Operation(summary = "Run AI orchestration for a ticket", description = "Debug endpoint that synchronously runs the classifier, extractor, retrieval, and resolution chain.")
    @PostMapping("/{id}/run")
    public Result<AiDecisionResult> runForTicket(@PathVariable("id") Long ticketId) {
        return Result.success(aiOrchestrationService.runForTicket(ticketId), TraceIdUtils.currentTraceId());
    }
